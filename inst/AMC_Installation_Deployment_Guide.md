...
## Overview
This guide provides with information on how to deploy AMC on a Kubernetes certified cluster.

Additionally there is information specific to common Kubernetes providers beyond generic instructions appliable to a standard cluster: Azure AKS, Amazon AWS EKS and OpenShift.
...
#### Processing
The following table expresses the CPU requirement in terms of vCPU units. This means that in order to run AMC you should have 4 cores assigned in total, where the distribution is as follows:
|                    | vCPU |
|--------------------|------|
| Main Application   | 1    |
...
#### Memory
We recommend having at least 12GiB total memory available for AMC if running on a single node.
...
### kubectl
For you to be able to connect with your Kubernetes cluster, you need to have _kubectl CLI_ installed on the machine you are using to deplkoy. If you are already managing a Kubernetes cluster, this should already be installed on your machine.
...
### Helm
In order to be able to deploy AMC you need to have Helm installed on the machine you are using to deploy.

Please follow the instructions for your platform provided [here](https://helm.sh/docs/intro/install/).
...
#### Standard Kubernetes and OpenShift
Please refer to your cluster administrator to provide you with the required access instructions.

#### Azure AKS
If you don't have _Azure CLI_ installed yet, follow the instructions [here](https://docs.microsoft.com/en-us/cli/azure/install-azure-cli?view=azure-cli-latest).
```bash
...
#### Amazon EKS
If you don't have _AWS CLI version 2_ installed yet, follor the instructions [here](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html).
```bash
...
If you don't have a provisioned _Ingress Controller_ you may follow the instructions for [NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/deploy/).
...
If you don't have one, and your deployment is on a standard certified Kubernetes cluster, there is provided an optional _Helm Chart_ dependency that deploys a third-party _PostgreSQL_ deployment alongside with AMC.
...
In order for this deployment to function, the cluster must be able to provision a persistent volume that supports `ReadWriteMany` mode (`RWX`).
Additionaly, to support dynamic provisioning, a StorageClass that provisions volumes in `RWX` mode must be made available.

#### Standard Kubernetes or OpenShift
Please refer to your cluster administrator to provide you with the required instructions.

#### Azure AKS
The easiest way to satisfy this is to create an _Azure Files_ dynamic provisioner.
Please follow the instructions available [here](https://docs.microsoft.com/en-us/azure/aks/azure-files-dynamic-pv).

---
**NOTE**

You only need to follow the instructions up to, and including, [Create a persistent volume claim](https://docs.microsoft.com/en-us/azure/aks/azure-files-dynamic-pv). You don't need to follow the instructions on _"Use the persistent volume"_.

---

#### Amazon EKS
The easiest way to satify this is to integrate _Amazon EFS CSI driver_.
Please follow the instruction available [here](https://docs.aws.amazon.com/eks/latest/userguide/efs-csi.html).
...
### SMTP Server
This product requires an SMTP Server in order to be able to send invitation emails, as well as password recovery emails. It is not possible to add users to the platform without a correctly configured SMTP Server.
...
Please refer to your cluster administration for this information.

### Secrets
These operations only need to be performed once, before the initial deployment.

Create image pull secret for the registry where the docker images are located at. This is dependent on where the images are stored, your cluster administrator should be able to assist you on this. Please keep note of the image pull secret name.
...
If you decided to automatically deploy an internal PostgreSQL server, set on `values.yaml`:
```yaml
...
- `treebase`: The treebase where user search will be performed.
- `emailAttribute`: The LDAP attribute on which the user's email is stored.
- `adminsGroup`: The LDAP group to which membership allows access to the platform as administrator.
- `usersGroup`: The LDAP group to which membership allows access to the platform as an app user.
- `enableLDAPS`: Enable LDAP over SSL
- `ldapsCertificateLocation`: LDAPS certificate file location, in PEM format, relative to the Chart's root. Validates the server's authenticity against this certificate. It can be a CA root certificate, or a self-signed certificate.
...
If traffic into the cluster is rooted by an external proxy employing a url rewrite strategy, effectively creating a virtual root, you need to deine the virtual root's path to on the configuration.
This option will NOT serve mobile capture under a virtual root, but is needed in order to correctly generate links for inviting new users, reset password, upload endpoint URLs, etc..
This value is used in conjuction with the value of hostname.
The value must begin with a forward slash, and terminate WITHOUT a traling slash
Example:
...
If you have installed `cert-manager` as you also need patch the ingress resource in order to have it configure the TLS certificate automatically.
```bash
...
This means you have previously installed the chart using Helm v2, and migrated to Helm v3. You will only have to fix this once, and the following upgrades should work as expected, with no errors being presented.
